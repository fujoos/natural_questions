...
`;function updateIframeStylesForDarkMode(isDarkMode){$("iframe").each(function(){const iframeDoc=this.contentDocument||this.contentWindow.document;$(iframeDoc.body).toggleClass("dark-mode",isDarkMode)})}$("#darkModeToggle").click(function(){const isDarkMode=$(this).is(":checked");updateIframeStylesForDarkMode(isDarkMode)});function updateTable(data,currentPage,pageSize){return new Promise(resolve=>{const tableBody=$("#data-rows");tableBody.empty();const isDarkMode=$("body").hasClass("dark-mode");data.forEach(function(row,index){const tr=$("<tr>");const rowIndex=(currentPage-1)*pageSize+(index+1);tr.append(`<td>${rowIndex}</td>`);tr.append(`<td>${row["question"]}</td>`);if(row["long_answers"].trim()){const iframeSrcdoc=`
