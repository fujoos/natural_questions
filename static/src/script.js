...
function showLoadingPlaceholders() {
  ...
  for (let i = 0; i < 5; i++) {
    // Creating 5 dummy rows
    const tr = $("<tr>").addClass("placeholder-row");
    const columns = 4; // Total number of columns in your table

    for (let j = 0; j < columns; j++) {
      const td = $("<td>");
      const numberOfPlaceholders = j === 0 ? 1 : 3; // First cell has 1 placeholder, others have 3

      for (let k = 0; k < numberOfPlaceholders; k++) {
        const placeholder = $("<div>").addClass("loading-placeholder");
